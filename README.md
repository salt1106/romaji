...
Romaji は変換後の日本語を元の入力欄へ挿入するために、macOSのアクセシビリティ権限を使います。

権限を許可したのに反映されない場合は、次を試してください。

1. Romaji を終了します。
2. `システム設定 > プライバシーとセキュリティ > アクセシビリティ` を開きます。
3. Romaji が登録されている場合は、一度削除します。
4. `/Applications/Romaji.app` をもう一度追加して許可します。
5. Romaji を起動し直します。

このアプリは無料配布のため ad-hoc 署名・未公証です。アップデート後にmacOS側の権限紐づけが古いアプリを指したままになることがあります。その場合は上の手順で再登録してください。
